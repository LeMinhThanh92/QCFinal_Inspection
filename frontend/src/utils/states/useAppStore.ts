...
export const useAppStore = create<AppState>((set) => ({
    ...
    initImages: (data) => set(() => {
        ...
        data.forEach(row => {
            if (row.Description && row.Image1) {
                ...
                const urls = row.Image1.split(',').filter(url => url.trim() !== '');
    ...
    initRecordedDefects: (defectsFromApi: any[]) => set(() => {
        const parsedDefects: RecordedDefect[] = defectsFromApi.map((d, index) => {
            const description = d.DefDescription || Object.values(d)[0]?.toString() || 'Unknown Defect';
            return {
                id: d.ID?.toString() || Date.now().toString() + index,
                type: 'Loaded Defect', // We use this generic type unless we can parse it
                code: description,
                major: 1 // Default
            };
        });
